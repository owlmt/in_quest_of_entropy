...
- Load (drives diverse IRQs): 16 parallel inbound HTTPS streams (ENA RX queues) + fio random NVMe I/O
  + stress-ng timers. Real traffic only; no synthetic data.
...
3. Start the load (16 curl streams + fio + stress-ng), then sudo timeout 300 bpftrace capture.bt > raw_events.csv
...
(observer effect), a known limitation; it adds noise, not spurious predictability.
